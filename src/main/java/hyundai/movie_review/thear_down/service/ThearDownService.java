...
// 데이터베이스 작업을 트랜잭션으로 묶는다. 일련의 작업이 모두 성공적으로 완료되거나, 하나라도 실패할 경우 모든 작업이 롤백되는 것을 보장합니다.
    public ThearDownResponse toggleThearDown(Long reviewId) {
        ...
        //이미 싫어요가 있을 경우
        if (thearDownRepository.existsByMemberIdAndReviewId(currenMember, review)) {
            //싫어요를 삭제 (싫어요 취소)
            thearDownRepository.deleteByMemberIdAndReviewId(currenMember, review);
            log.info("싫어요 삭제 완료");

            applicationEventPublisher.publishEvent(
                    new ThearDownScoreEvent(this, currenMember, false));
            return ThearDownResponse.of("싫어요 삭제 완료");
        } else {
            //싫어요가 없으면 새로 생성
            ThearDown thearDown = ThearDown.builder()
                    .memberId(currenMember)
                    .reviewId(review)
                    .build();
            ...
            applicationEventPublisher.publishEvent(
                    new ThearDownScoreEvent(this, currenMember, true));
